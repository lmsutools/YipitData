...
import { sectors, companies, kpis, kpiEstimates } from '../db/schema.js'
import { eq, and, gte, lte, desc, ilike, or } from 'drizzle-orm'
import { z } from 'zod'

const server = new Server(
  { name: 'yipitdata-kpi', version: '1.0.0' },
  { capabilities: { tools: {} } },
)
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'list_sectors',
      description: 'List all available sectors (e.g. Footwear, Apparel, Electronics) with company counts.',
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          sector: { type: 'string', description: 'Sector slug (e.g. "footwear")' },
    ...
    {
      name: 'get_company',
      description: 'Get detailed information about a specific company including its latest MTD KPI snapshot.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'list_kpis',
      description: 'List all available KPI definitions (e.g. GMV, Units Sold, ASP).',
      inputSchema: { type: 'object', properties: {}, required: [] },
    },
    {
      name: 'get_estimates',
      description: 'Get time-series KPI estimates for a company. Returns historical and/or MTD estimates. Use dateFrom/dateTo (YYYY-MM-DD) to filter by period.',
      inputSchema: {
        type: 'object',
        properties: {
          companyId: { type: 'number', description: 'Company ID' },
    ...
    },
    {
      ...
      description: 'Get the latest Month-to-Date (MTD) estimates for a company, showing current month performance.',
      inputSchema: {
        type: 'object',
        properties: {
          companyId: { type: 'number', description: 'Company ID' },
    ...
    },
    {
      ...
      description: 'Compare a KPI value between two periods for year-over-year (YOY) or month-over-month (MOM) analysis. Returns absolute and percentage delta.',
      inputSchema: {
        type: 'object',
        properties: {
          companyId: { type: 'number', description: 'Company ID' },
    ...
    },
    {
      ...
      description: 'Publish a new KPI estimate for a company. Use estimateType="mtd" for intra-month updates.',
      inputSchema: {
        type: 'object',
        properties: {
        ...
        required: ['companyId', 'kpiId', 'periodMonth', 'estimateValue', 'estimateType'],
      },
    },
  ],
}))
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'get_estimates': {
        const { companyId, kpiId, dateFrom, dateTo, type = 'all' } = args as {
          companyId: number; kpiId?: number; dateFrom?: string; dateTo?: string; type?: string
        }
        ...
        const rows = await db
          ...
          .where(and(...conds))
          .orderBy(kpiEstimates.kpiId, kpiEstimates.periodMonth)
        return { content: [{ type: 'text', text: JSON.stringify(rows, null, 2) }] }
      }

      case 'get_mtd_snapshot': {
        const { companyId, kpiId } = args as { companyId: number; kpiId?: number }
      ...
      case 'compare_periods': {
        const { companyId, kpiId, period1, period2 } = args as {
          companyId: number; kpiId: number; period1: string; period2: string
        }
        const [e1] = await db.select().from(kpiEstimates)
          .where(and(eq(kpiEstimates.companyId, companyId), eq(kpiEstimates.kpiId, kpiId), eq(kpiEstimates.periodMonth, period1), eq(kpiEstimates.estimateType, 'historical')))
          .limit(1)
        const [e2] = await db.select().from(kpiEstimates)
          .where(and(eq(kpiEstimates.companyId, companyId), eq(kpiEstimates.kpiId, kpiId), eq(kpiEstimates.periodMonth, period2), eq(kpiEstimates.estimateType, 'historical')))
          .limit(1)
        if (!e1 || !e2) return { content: [{ type: 'text', text: 'One or both periods not found' }] }
        const v1 = parseFloat(e1.estimateValue)
        const v2 = parseFloat(e2.estimateValue)
        const delta = v2 - v1
        const pct = v1 !== 0 ? ((delta / v1) * 100).toFixed(2) : null
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({ period1, value1: v1, period2, value2: v2, absoluteDelta: delta, percentageDelta: pct ? `${pct}%` : 'N/A' }, null, 2),
          }],
        }
      }

      case 'publish_estimate': {
        const { companyId, kpiId, periodMonth, estimateValue, estimateType } = args as {
          companyId: number; kpiId: number; periodMonth: string; estimateValue: number; estimateType: 'historical' | 'mtd'
        }
        const [inserted] = await db.insert(kpiEstimates).values({
          companyId, kpiId, periodMonth,
          estimateValue: String(estimateValue),
          estimateType,
          asOfTimestamp: estimateType === 'mtd' ? new Date() : null,
        ...
        .returning()
        return { content: [{ type: 'text', text: `Published estimate id=${inserted!.id}` }] }
      }
